...
@Component({
  selector: 'app-returned-login',
  templateUrl: './returned-login.component.html',
  styleUrls: ['./returned-login.component.scss']
})
export class ReturnedLoginComponent implements OnInit {
...
loading = false;
  ...
  ngOnInit() {
    let resp1= this.http.get("https://api.team4loans.in/api/loan_add/login-return")
...
console.log(this.app_rvd);
  ...
  openModal(bar_code) {
      ...
      const modalDialog = this.matDialog.open(ReturnedDialogComponent, dialogConfig);
    } 
    refresh() {
